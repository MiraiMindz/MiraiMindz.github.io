...
function ClassContent(props: ClassContentProps) {
  ...
  return (
    <div>
      ...
      <ClassChapter1/>
      <ClassChapter2 />
      <BackNextChapters nextChapLink="/classes/pyless/4" prevChapLink="/classes/pyless/2" />
    </div>
  )
}
    ...
    <div className={(props.state ? "blur-sm" : "pt-0 md:pt-16 min-h-screen grow font-leagueGothic")}>
      ...
      <div className="px-8 relative hidden md:flex flex-row">
        ...
        <div className="w-0 grow px-2 text-justify">
          <ClassContent chapterTitle="Fundamentos do Python II" refLinksList={props.refLinksList} />
        </div>
        <div className="w-20">
          <CapSumm currCap={3} />
        </div>
...
export function PyLess3(props: GenericStateProps) {
  return (
    <div className='flex flex-col bg-ctp-base text-ctp-text'>
      <Header state={props.state} funcState={props.funcState} title="Fund. II" sums={<CapsSumPhone />} caps={<CapsLinkPhone currCap={3}/>} themeFunc={props.themeFunc} themeState={props.darkLightSwitch} />
